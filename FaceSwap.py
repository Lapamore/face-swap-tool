...
class FaceSwapper():
    def __init__(self, img_destination_path, img_source_path):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Программа для замены лица на изображении.')
    parser.add_argument('--destination', type=str, required=True, help='Путь к изображению, на котором будет заменено лицо')
    parser.add_argument('--source', type=str, required=True, help='Путь к изображению, с которого будет взято лицо')

    args = parser.parse_args()

    face_swapper = FaceSwapper(img_destination_path=args.destination,
                               img_source_path=args.source)
    try:
        face_swapper.run()
